...
event.on('get-product-specials', (product) => {
    ...
    console.log('executing product specials')
});
...
event.on('get-product-combined-specials', (product) => {
    ...
    console.log('executing COMBINED product specials')
});
...
Basket.saveCustomerBasket = (req, result) => {
    ...
    dbConn.query('INSERT INTO loyalty_program.tblbasketinfo(basket_id, customer_id, purchase_date, total_amount, payment_method)VALUES(?, ?, ?, ?, ?)', [basket_id, customer_id, purchase_date, total_amount, payment_method], (err, res) => {
        if (err) {
...
Basket.getProductPrices = (product, result) => {
    ...
    const query = 'SELECT mstn.id, mstn.selling_incl_1, mstn.special_price_incl, COALESCE(inv.description_1, inv.description_2) AS description FROM loyalty_program.tblmultistoretrn mstn JOIN loyalty_program.tblinventory inv ON mstn.item_code = inv.item_code WHERE COALESCE(inv.description_1, inv.description_2) = ?'
...
Basket.checkLoyaltyCustomer = (customerId, result) => {
    ...
    const customerID = customerid
        ...
        dbConn.query('SELECT CustomerID, LoyaltyTier FROM loyalty_program.tblloyaltycustomers WHERE CustomerID = ?', [customerID], (err, res) => {
            if (err) {
...
Basket.getProductSpecials = (products, result) => {
    ...
    const query = `
        ...
        JOIN 
            loyalty_program.tblspecialitems spi
        ON 
            sp.special_id = spi.special_id
        WHERE 
            sp.special_type = 'Special' 
            AND sp.isActive = 1 
...
Basket.getProductCombinedSpecials = (products, result) => {
    ...
    const query = `
        ...
        WHERE 
            ...
            AND sp.isActive = 1 
            AND spcg.product_description IN ('Apple', 'Orange', 'Banana') 
            AND sp.start_date <= CURDATE() 
            AND sp.expiry_date >= CURDATE();
    `;
        ...
        return new Promise((resolve, reject) => {
            dbConn.query(query, [product], (err, res) => {
                if (err) {
                    ...
                    //console.log(`No specials found for product: ${product}`);
                    resolve({ product, message: "item is not linked to a combined special" });
                } else {
                    resolve({ product, specials: res, message: "item is linked to combined special" });
                }
            });
        });
    });
    ...
    .then((results) => {
        console.log(
            "Returned COMBINED Product Specials: ",
            JSON.stringify(results, null, 2) // Pretty-print the JSON
        );
    ...
    .catch((err) => {
        console.error("ERROR FETCHING COMBINED PRODUCT SPECIALS: -- --", err);
        result(err, null);
    });
};
...
Basket.saveFinalTransaction = (basketid, customerid, product, quantity, sellingPrice, newDiscountedPrice, totalamount, newTotalDiscBasketAmount, purchasedate, result) => {
        dbConn.query('INSERT INTO loyalty_program.tblcompletetransaction (basket_id, customer_id, purchased_product, quantity, product_amount, product_discounted_amount, total_basket_amount, total_disc_basket_amount, payment_method, purchase_date)VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [basketid, customerid, product, quantity, sellingPrice, newDiscountedPrice, totalamount, newTotalDiscBasketAmount, purchasedate], (err, res) => {
            if (err) {
...
//         dbConn.query('UPDATE loyalty_program.tblcustomertiers SET loyalty_tier = ?, eligibility = ?, rewards = ?, discounts = ?, min_spending_amount = ?, max_spending_amount = ? WHERE uid =  ?', [basketid, customerid, product, quantity, sellingPrice, newDiscountedPrice, totalamount, newTotalDiscBasketAmount, purchasedate], (err, res) => {
//             if (err) {
//                 //console.log('Error while checking the product specials for the purchased item' + err);
//                 result(null, err);
//             } else {
...
//         dbConn.query('DELETE FROM loyalty_program.tblcustomertiers WHERE uid =  ?', [req.params.uid], (err, res) => {
//             if (err) {
//                 //console.log('Error while checking the product specials for the purchased item' + err);
//                 result(null, err);
//             } else {
